...
NumaProf also uses the binary instrumentation  (i.e., PIN) to collect and identify local and remote memory accesses~\cite{valat:2018:numaprof}. \todo{Why NumaProf does not perform as good as \NP{}??? Are they helpful to identify the issues?} NumaProf also relies on a specific thread binding to detect remote accesses, which shares the same shortcoming as other existing work~\cite{XuNuma, 7847070}.   
